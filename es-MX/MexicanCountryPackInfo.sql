...
IF EXISTS (SELECT 1 FROM CountryPack WHERE [Option] = N'T' And [Name] = N'es-MX') 
BEGIN 
    UPDATE [CountryPack] SET [Version] = N'2020.5.0.3.0003'  
    WHERE [Option] = N'T' And [Name] = N'es-MX' 
END 
ELSE 
BEGIN 
    INSERT INTO [CountryPack] ([Option], [Name], [Description], [Version]) 
    VALUES (N'T', N'es-MX', N'Spanish (Mexico)', N'2020.5.0.3.0003')  
END 
GO
