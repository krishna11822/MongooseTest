...
IF EXISTS (SELECT 1 FROM CountryPack WHERE [Option] = N'T' And [Name] = N'es-AR') 
BEGIN 
    UPDATE [CountryPack] SET [Version] = N'2020.5.0.3.0003'  
    WHERE [Option] = N'T' And [Name] = N'es-AR' 
END 
ELSE 
BEGIN 
    INSERT INTO [CountryPack] ([Option], [Name], [Description], [Version]) 
    VALUES (N'T', N'es-AR', N'Spanish (Argentina)', N'2020.5.0.3.0003')  
END 
GO
