-- Modified by issue 253490 to handle fs_sro/fs_incident sharing.
...
SELECT nsm.FromSite, nsm.FromNoteToken, nsm.LocalNoteToken, obn.RefRowPointer, obn.ObjectNoteToken, NEWID(), ROW_NUMBER() OVER (PARTITION BY obn.SpecificNoteToken, NoteHeaderToken ORDER BY SpecificNoteToken, NoteHeaderToken)
FROM notessitemap nsm
INNER JOIN ObjectNotes obn ON
...
ALTER TABLE NotesSiteMap DROP CONSTRAINT PK_NotesSiteMap
GO
ALTER TABLE NotesSiteMap ADD CONSTRAINT PK_NotesSiteMap PRIMARY KEY (FromSite, FromNoteToken, TypeOfNote, ToSite, LocalNoteToken)
GO
