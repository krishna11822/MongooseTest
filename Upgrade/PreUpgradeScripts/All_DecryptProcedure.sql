...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[DecryptProcedureSp]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DecryptProcedureSp]
GO

CREATE PROCEDURE [dbo].[DecryptProcedureSp]
...
SET @fake_01='ALTER PROCEDURE '+ quotename(@procedure) +' WITH ENCRYPTION AS
...
SET @fake_01='CREATE PROCEDURE '+ quotename(@procedure) +' WITH ENCRYPTION AS
'+REPLICATE('-', 40003 - @procNameLength)
--start counter
SET @intProcSpace=1
