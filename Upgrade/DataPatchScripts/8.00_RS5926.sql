...
SET Parameters = 'IDO("SqlColumns")
ACTION(INSERT)
SETPROPVALUES("columnName" = P(Batch, V(BatchRowIndex), "Name"), "tableName" = P(Batch, V(BatchRowIndex), "TableIDOName"), "tableSchema" = "dbo", "dataType" = P(Batch, V(BatchRowIndex), "ColumnDataType"), "decimalPos" = V(nullvalue), "UBDecPosUsed" = "0", "dataLength" = P(Batch, V(BatchRowIndex), "PropertyLength"), "UBLengthUsed" = "0", "isNullable" = V(ColumnNullable), "defaultValue" = V(nullvalue), "systemDataType" = V(nullvalue), "derShadowColumnName" = V(nullvalue))'

WHERE EventHandlerRowPointer = @EHRP AND Sequence = 40
